...
- **Encoding**:
  ...
  - `4'h8`: Environment call from U-mode (ECALL) — **not reachable in this M-mode-only core**
  - `4'h9`: Environment call from S-mode — **not reachable in this M-mode-only core**
  - `4'hB`: Environment call from M-mode (the only ECALL trap code generated by this core)
  - `4'hC`: Instruction page fault
  - `4'hD`: Load page fault
  - `4'hF`: Store/AMO page fault

#### Privileged Instructions Implemented as NOPs
- **WFI** (Wait For Interrupt): Treated as a NOP (no power management; execution continues immediately)
- **SFENCE.VMA** (Supervisor Fence for Virtual Memory): Treated as a NOP (no MMU/TLB to flush)
